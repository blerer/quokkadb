...
impl ManifestState {
    ...
    pub fn apply(&self, edit: &ManifestEdit) -> Self {
        match edit {
            ...
            ManifestEdit::Snapshot(_) => { panic!("Snapshots should not be applied to an LSMTree"); },
        }
    }
}
...
pub enum ManifestEdit {
    ...
    Flush { oldest_log_number: u64, sst: Arc<SSTableMetadata> },
...
impl ManifestEdit {
    ...
    pub fn try_from_vec(input: &[u8]) -> Result<ManifestEdit> {
        ...
        match edit {
            0 => {
                Ok(ManifestEdit::Snapshot(Arc::new(ManifestState::read_from(&reader)?)))
            },
            1 => {
            ...
            5 => {
                ...
                Ok(ManifestEdit::Flush { oldest_log_number, sst })
            }
            6 => {
                let next_file_number = reader.read_varint_u64()?;
                Ok(ManifestEdit::FilesDetectedOnRestart { next_file_number })
            }
            _ => Err(Error::new(ErrorKind::InvalidData, format!("ManifestEdit: {}", edit))),
        }
    }
}
...
mod tests {
    ...
    fn test_wal_and_manifest_rotation_serialization() {
        ...
        check_edit_serialization_roundtrip(ManifestEdit::ManifestRotation { manifest_number: 456 });
    }
    ...
    //
    //
    //     let edit = ManifestEdit::Snapshot(Arc::new(create_lsm_tree()));
    //     check_edit_serialization_roundtrip(edit);
    // }
    ...
    fn test_apply_create_and_drop_collection() {
        ...
        assert_eq!(Some(42), tree.catalog.get_collection_id(&"docs".to_string()));

        let tree = tree.apply(&ManifestEdit::DropCollection {
            name: "docs".to_string(),
        });
        assert!(tree.catalog.get_collection_id(&"docs".to_string()).is_none());
    }
    ...
    fn test_apply_wal_and_manifest_rotation() {
        ...
        let tree = tree.apply(&ManifestEdit::ManifestRotation { manifest_number: 150 });
